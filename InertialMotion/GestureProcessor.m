...
- (void)processGesture2DWithSamples:(const Sample2D *)samples
  ...
  for (int i = 0; i < count; i++) {
    Sample2D sample = samples[i];
    Sample2D rescaledSample;
    rescaledSample.x = (sample.x-minX)/clippedSize;
    rescaledSample.y = (sample.y-minY)/clippedSize;
    rescaledSamples[i] = rescaledSample;
  }
  ...
  for (int i = 1; i < count; i++) {
    ...
    //samples are normalized from 0 to 1, we want to separate into 3x3 board
    //multiply sample by 3, getting number between 0 and 3
    //make each into an int, ensuring that they fit into one of the 9 sectors by making them fit into the third sector at most
