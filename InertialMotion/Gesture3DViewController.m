...
- (void)accumulateMotion:(CMDeviceMotion *)motion
{
  ...
  acceleration = GLKQuaternionRotateVector3(attitude,acceleration);  // rotate acceleration from instantaneous coordinates into persistent coordinates
  
  // -- TASK 2B --
  // integrate acceleration into _velocity and _velocity into _position
  double alpha = 0.99;
  _velocity = GLKVector3Add(GLKVector3MultiplyScalar(_velocity,pow(M_E,-alpha * dt * .5)),GLKVector3MultiplyScalar(acceleration, dt * 0.5));
  _position = GLKVector3Add(GLKVector3MultiplyScalar(_position,pow(M_E,-alpha * dt)),GLKVector3MultiplyScalar(_velocity, dt));
  _velocity = GLKVector3Add(GLKVector3MultiplyScalar(_velocity,pow(M_E,-alpha * dt * .5)),GLKVector3MultiplyScalar(acceleration, dt * 0.5));
